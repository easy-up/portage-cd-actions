...
su podman -s sh -c "git config --global --add safe.directory $GITHUB_WORKSPACE"
...
if [ "$CONTAINER_REGISTRY" != "" ] && [ "$REGISTRY_USER" != "" ] && [ "$REGISTRY_TOKEN" != "" ]; then
	...
	su podman -s sh -c "echo \"$REGISTRY_TOKEN\" | podman login \"$CONTAINER_REGISTRY\" -u \"$REGISTRY_USER\" --password-stdin"
fi

su podman -s sh -c "workflow-engine run all --verbose --semgrep-experimental --cli-interface podman"
